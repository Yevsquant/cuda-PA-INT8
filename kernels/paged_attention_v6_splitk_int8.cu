// Stage 6 — Flash-Decoding split-K with INT8 KV-cache (paged decode).
//
// Same split-K structure as Stage 4 (paged_attention_v4_splitk.cu): one block
// per (seq, head, split) emits an un-normalized fp32 partial, then a reduce
// kernel merges them. The only change vs Stage 4 is the partition kernel reads
// an INT8 KV cache with per-token fp32 scales, dequantized inline exactly as in
// Stage 5 (qk_dot_int8 then ·s_k; s_v folded into the per-token weight).
//
// The reduce kernel operates purely on fp32 partials, so it is identical to
// Stage 4's; it is re-declared here in the anonymous namespace (Stage 4's copy
// lives in another translation unit's anonymous namespace and isn't linkable).
...
__global__ void paged_decode_splitk_int8_partition_kernel(
    ...
    int block_size, int max_blocks, int x, int num_splits)
{
    ...
    // Asymmetric-K correction (see Stage 5): q_sum = Σ_d q, the same for every
    // key in this split, so compute once. Only needed when k_zeros is given.
    float q_sum = 0.f;
    if (k_zeros != nullptr) {
        float ql = 0.f;
...
// Identical to Stage 4's reduce kernel — operates on fp32 partials only.
__global__ void paged_decode_splitk_int8_reduce_kernel(
    ...
    c10::optional<torch::Tensor> k_zeros)
{
    ...
    TORCH_CHECK(block_size % 16 == 0 && TILE % block_size == 0 &&
                ...
                "block_size must divide TILE (multiple of 16) and TILE must divide PARTITION_SIZE");
    ...
    paged_decode_splitk_int8_partition_kernel<<<pgrid, threads, smem, stream>>>(
    ...
    paged_decode_splitk_int8_reduce_kernel<<<rgrid, threads, 0, stream>>>(
